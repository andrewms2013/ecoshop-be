...
export const catalogBatchProcess: SQSHandler = async (event) => {
  try {
    const parsedProducts: Omit<ShopItem, 'id'>[] = event.Records.map(({ body }) => JSON.parse(body));
    ...
    for (const product of parsedProducts) {
      await withDbConnection(async (client) => {
        try {
          ...
          console.log(`Product with name ${product.name} was imported under id ${insertedProduct.id}`);
        } catch (error) {
          await client.query('ROLLBACK');

          console.error(`Product with name ${product.name} was not imported because of an error`);
          console.error(error);
